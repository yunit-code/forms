      ...
      >
        ...
        >
          ...
          <span class="label-content">{{
            IDM.env_dev ? propData.label : "单行输入框"
          }}</span>
        </div>
        <div
          class="fic-input-box"
...
export default {
  ...
  methods: {
    ...
    convertAttrToInputFoucsStyle() {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " input:focus",
        styleObject
      );
    },
    /**
    ...
    convertAttrToInputErrorStyle() {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " input.error-state",
        styleObject
      );
    },
    /**
    ...
    verifyInputValue(type) {
      ...
      if (this.propData.required && thisInputVal == "") {
        ...
        this.propData.email &&
        !window.IDM.validate("isEmail", thisInputVal)
      ) {
        result.success = false;
        result.message =
          this.propData.emailText || this.propData.label + "格式错误";
      } else if (
        this.propData.mobile &&
        !window.IDM.validate("isMobile", thisInputVal)
      ) {
        result.success = false;
        result.message =
          this.propData.mobileText || this.propData.label + "格式错误";
      } else if (
        this.propData.url &&
        !window.IDM.validate("isUrl", thisInputVal)
      ) {
        result.success = false;
        result.message =
          this.propData.urlText || this.propData.label + "格式错误";
      } else if (
        this.propData.ipAddress &&
        !window.IDM.validate("isIpAddress", thisInputVal)
      ) {
        result.success = false;
        result.message =
          this.propData.ipAddressText || this.propData.label + "格式错误";
      } else if (
        this.propData.macAddress &&
        !window.IDM.validate("isMacAddress", thisInputVal)
      ) {
        result.success = false;
        result.message =
