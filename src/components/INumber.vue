      ...
      >
        ...
        >
          ...
          <span class="label-content">{{
            IDM.env_dev ? propData.label : "数字输入框"
          }}</span>
        </div>
        <div
          class="fic-input-box"
...
export default {
  ...
  methods: {
     ...
     * 把属性转换成数字框的样式(默认)
     */
    convertAttrToNumberDefaultStyle() {
      var styleObject = {};
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number",
        styleObject
      );
    },
    /**
    ...
    convertAttrToNumberFoucsStyle() {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number.ant-input-number-focused,#"+this.moduleObject.id + " .ant-input-number:hover",
        styleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number-handler-wrap,#"+this.moduleObject.id + " .ant-input-number-handler-down",
        {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number-handler:active",
        {
    ...
    convertAttrToNumberErrorStyle() {
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number.error-state",
        styleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + " .ant-input-number.error-state .ant-input-number-handler-wrap,#"+this.moduleObject.id + " .ant-input-number.error-state .ant-input-number-handler-down",
        {
          ...bStyleObject
        }
      );
    ...
    convertAttrToStyleObject() {
      //默认值
      this.thisValue = this.propData.defaultValue || (this.propData.defaultValue==0?0:null);
      this.convertAttrToNumberDefaultStyle();
      this.convertAttrToNumberFoucsStyle();
      this.convertAttrToReadOnlyFontStyle();
      this.convertAttrToLabelFontStyle();
      this.convertAttrToErrorMsgFontStyle();
      this.convertAttrToNumberErrorStyle();
    },
    formatterHandle(value){
      var resValue = value;
      if(this.propData.formatterFunction&&this.propData.formatterFunction.length>0){
        try {
          resValue = window[this.propData.formatterFunction[0].name]&&window[this.propData.formatterFunction[0].name].call(this,{...this.propData.formatterFunction[0].param,moduleObject:this.moduleObject,value:value});
        } catch (error) {
        }
      }
      return resValue;
    ...
    parserHandle(value){
      var resValue = value;
      if(this.propData.parserFunction&&this.propData.parserFunction.length>0){
        try {
          resValue = window[this.propData.parserFunction[0].name]&&window[this.propData.parserFunction[0].name].call(this,{...this.propData.parserFunction[0].param,moduleObject:this.moduleObject,value:value});
        } catch (error) {
        }
      }
      return resValue;
    ...
    getContextValue() {
      ...
      if (!this.verifyInputValue().success) {
        ...
        this.$refs["inumber" + this.moduleObject.id].focus();
      } else {
    ...
    verifyInputValue(type) {
      ...
      //这里开始判断执行是否需要校验
      if (this.propData.required && (thisInputVal == ""||thisInputVal == "null")) {
        //必填
        ...
        parseFloat(thisInputVal) < this.propData.minSizeNumber
      ) {
        result.success = false;
        result.message =
          this.propData.minSizeText ||
          this.propData.label + "最小值为" + this.propData.minSizeNumber;
      } else if (
        this.propData.maxSize &&
        parseFloat(thisInputVal) > this.propData.maxSizeNumber
      ) {
        result.success = false;
        result.message =
          this.propData.maxSizeText ||
          this.propData.label + "最大值为" + this.propData.maxSizeNumber;
      }
